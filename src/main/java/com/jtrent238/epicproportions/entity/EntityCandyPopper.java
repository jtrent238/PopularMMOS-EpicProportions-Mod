...
public class EntityCandyPopper  extends EntityMob
{
...
*/
     ...
     * Called when the entity is attacked.
     * @param damageSource 
     * @param entityAttackedBy 
     */
    
    public boolean attackEntityFrom(DamageSource damageSource, EntityPlayer entityAttackedBy) {
    	if (getHealth() <= 0.0F)
    	{
    	    ...
    	    if (entityAttackedBy instanceof EntityPlayer)
    	    {
    	        ((EntityPlayer)entityAttackedBy)
    	              .addStat(Achievements.achievementpopped, 1);
    	    }
    	}
		return true;
	}
