...
public class TeleporterEpic extends Teleporter {
	...
	public TeleporterEpic(WorldServer worldserver) {
		super(worldserver);
			...
			for(int l = -2; l <= 2; ++l){
				for(int i1 = -2; i1 <= 2; ++i1){
					for(int j1 = -1; j < 3; ++j1){
						...
						int i2 = k + i1 * b1 - l * b0;
						boolean flag = j1 < 0;
						this.worldServerInstance.setBlock(k1, l1, i2, flag ? BlockLoader.BlockOfEpicProportions : Blocks.air);
					}
				}
			}
	...
	public boolean placeInExistingPortal(Entity entity, double par2, double par4, double par6, float par8){
		...
		if(this.destinationCoordinateCache.containsItem(j1)){
			TeleporterEpic.PortalPosition portalposition = (Teleporter.PortalPosition)this.destinationCoordinateCache.getValueByKey(j1);
			d3 = 0.0D;
		...
		if (d3 >= 0.0D){
			if(flag){
				this.destinationCoordinateCache.add(j1, new TeleporterEpic.PortalPosition(i, j, k, this.worldServerInstance.getTotalWorldTime()));
